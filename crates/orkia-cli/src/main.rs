...
#[derive(Subcommand)] enum ReviewCommand { Plan { #[arg(long)] path: PathBuf, #[arg(long)] checkpoint: Option<String> }, Show { #[arg(long)] plan: String } }
...
fn run(cli: Cli) -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Ledger { command: LedgerCommand::Verify } => { let state = read_state(&root)?; let events = git.ledger_store().read_all()?; let actors = BTreeMap::from([(state.actor.id.clone(), state.actor)]); verify_chain(&events, &actors)?; println!("verified {} signed ledger events", events.len()); }
...
fn handle_session(command: SessionCommand, git: &LibGit2Repository, root: &Path, secrets: &FileSecrets) -> Result<()> { match command {
    ...
    SessionCommand::Agent { provider, args } => { let (_, ledger) = open_ledger(git, root, secrets)?; let program = match provider { Provider::Codex => "codex", Provider::Claude => "claude" }; let output = std::process::Command::new(program).args(&args).output().map_err(|e| OrkiaError::External(format!("cannot run {program}: {e}")))?; let stdout = String::from_utf8_lossy(&output.stdout).into_owned(); ledger.append(CaptureEvent::Command { command: std::iter::once(program).chain(args.iter().map(String::as_str)).collect::<Vec<_>>().join(" "), exit_code: output.status.code(), stdout: stdout.clone(), stderr: String::from_utf8_lossy(&output.stderr).into_owned() })?; let adapter: Box<dyn ProviderAdapter> = match provider { Provider::Codex => Box::new(CodexAdapter), Provider::Claude => Box::new(ClaudeAdapter) }; for event in adapter.capture(&stdout) { ledger.append(event)?; } if !output.status.success() { return Err(OrkiaError::External(format!("{program} session failed"))); } println!("{} agent session captured", adapter.provider_name()); }
...
fn handle_review(command: ReviewCommand, git: &LibGit2Repository, root: &Path) -> Result<()> { match command { ReviewCommand::Plan { path, checkpoint } => { let state = read_state(root)?; let content = fs::read_to_string(&path).map_err(|e| OrkiaError::External(e.to_string()))?; let events = git.ledger_store().read_all()?; let source_events = events.iter().map(|event| event.unsigned.id.clone()).collect(); let atoms = extract_atoms(&ChangedFile { path: path.to_string_lossy().into_owned(), changed_start: 1, changed_end: content.lines().count() as u32, content, source_events }); let plan = plan(PlanningInput { checkpoint: checkpoint.unwrap_or(git.head_commit()?), atoms, dependencies: vec![], coverage_milli: 1000, minimum_coverage_milli: 950, minimum_confidence_milli: 800, source_events: BTreeSet::new() }); let destination = root.join("orkia/plans").join(format!("{}.json", plan.id.0)); write_json(&destination, &plan)?; println!("review plan {}: {} unit(s), session {}", plan.id.0, plan.units.len(), state.id.0); }
